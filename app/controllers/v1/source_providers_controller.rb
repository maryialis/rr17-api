module V1
  class SourceProvidersController < ApiBaseController
    ...
    def destroy
      rec = SourceProvider.find_by_id(params[:id])
      render json: { errors: 'Unauthorized to destroy source provider', status: 401 }, status: 401 && return unless rec
      render json: { errors: 'Unauthorized to destroy source provider', status: 401 }, status: 401 && return unless rec.user_id == current_user.id
      rec.destroy
    end
